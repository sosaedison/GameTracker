...
class MainView(tk.Tk):
    ...
    def startTracker(self):
        subprocess.Popen([self.TRACKER_URI])
        return self.setTrackerId()
    
    def setTrackerId(self):
        try:
            for process in psutil.process_iter():
                if process.name() == "Tracker.exe":
                    config = self.getsettings()
                    config['trackerstarted'] = process.create_time()
    ...
    def trackerIsRunning(self):
        try:
            trackerstarted = self.getconfig("trackerstarted")
            for process in psutil.process_iter():
                if process.create_time() == trackerstarted:
                    return True
            return False
        except Exception as ex:
            print(ex)
    ...
    def getconfig(self, key):
        with open(r"AppTracker\bin\tracker_config.json", "r") as tracker_config:
